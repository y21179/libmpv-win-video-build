#!/bin/bash
# 基于 zhongfly/mpv-winbuild 的 build.sh，简化为单架构构建。
# 用法: bash build.sh -t 64-v3 -c clang -s true
set -x
...
main "${target:-64-v3}" "${compiler:-clang}" "${simple_package:-true}"
